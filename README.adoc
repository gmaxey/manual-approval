...
* All parameter input values provided by a workflow approver in JSON format use: needs syntax of `${{needs..outputs.approvalInputValues}}`

* A specific approval parameter input value use: `${{ fromJSON(needs..outputs.approvalInputValues). }}`.
