...
These approval parameter input values can be accessed in subsequent jobs using the `outputs` context. For example, to return in JSON format:

* All parameter input values provided by a workflow approver use: `needs` syntax of `${{needs.<approvalJobName>.outputs.approvalInputValues}}`

* A specific approval parameter input value use:  `${{ fromJSON(needs.<approvalJobName>.outputs.approvalInputValues).<parameterName> }}`.
